          ...
          <div className="grid gap-0 md:grid-cols-[230px_1fr]">
            ...
            <main className="p-5 md:p-8">
              ...
              {step === 'criteria' && <section className="mt-8 max-w-4xl">
                ...
                {selectedCriteria.length > 0 && <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><span className="text-[11px] font-semibold text-slate-700">Rate your selected criteria</span><span className="text-[10px] text-slate-400">{answeredCount} rated</span></div><div className="mt-3 space-y-4">{selectedCriteria.map((key) => { const criterion = current.allPrinciples.find((item) => item.key === key); return <div key={key} className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="grid gap-2 sm:grid-cols-[1fr_1fr]"><span className="text-[11px] text-slate-600">{criterion.label}</span><div className="flex gap-1">{[1,2,3,4,5].map((value) => <button key={value} onClick={() => setRatings((old) => ({ ...old, [key]: value }))} className={`flex-1 rounded-lg py-1.5 text-[10px] font-semibold ${ratings[key] === value ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-200'}`} title={ratingLabels[value - 1]}>{value}</button>)}</div></div><input value={criterionNotes[key] || ''} onChange={(event) => setCriterionNotes((old) => ({ ...old, [key]: event.target.value }))} placeholder={`Why did ${criterion.label.toLowerCase()} matter here?`} className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] outline-none placeholder:text-slate-300 focus:ring-2 focus:ring-blue-100" /></div>})}</div></div>}
